...
import { useNavigationSideMenu } from './useNavigationSideMenu';
...
const addActiveStateToItems = (
  ...
  items.map((item, index) => {
    ...
    if ('items' in item) {
      return {
        expandedByDefault: activeHref ? item.items.some((subItem) => isItemActive(activeHref, subItem)) : index === 0,
        ...item,
    ...
    return {
      ...item,
      isActive: activeHref ? isItemActive(activeHref, item) : index === 0 && isFirstRecursion
    };
  });
...
const NavigationSideMenu = ({
  ...
  return <SideMenu {...sideMenuProps} items={addActiveStateToItems(items, activeHref)} />;
};
